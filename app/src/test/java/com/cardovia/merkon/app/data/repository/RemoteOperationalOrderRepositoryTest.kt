...
class RemoteOperationalOrderRepositoryTest {
    ...
    private lateinit var api: MerkonApi
    ...
    fun `HTTP 502 maps to IOException`() = runTest {
        val errorResponse = Response.error<com.cardovia.merkon.app.data.model.OrderPaymentCollectionResponse>(502, "Bad Gateway".toResponseBody("text/plain".toMediaTypeOrNull()))
    ...
    fun `HTTP 503 maps to IOException`() = runTest {
        val errorResponse = Response.error<com.cardovia.merkon.app.data.model.OrderPaymentCollectionResponse>(503, "Service Unavailable".toResponseBody("text/plain".toMediaTypeOrNull()))
        coEvery { api.collectPayment(any(), any()) } returns errorResponse
    ...
    fun `HTTP 504 maps to IOException`() = runTest {
        val errorResponse = Response.error<com.cardovia.merkon.app.data.model.OrderPaymentCollectionResponse>(504, "Gateway Timeout".toResponseBody("text/plain".toMediaTypeOrNull()))
        coEvery { api.collectPayment(any(), any()) } returns errorResponse
    ...
    fun `structured 4xx maps to ApiException`() = runTest {
        ...
        val errorResponse = Response.error<com.cardovia.merkon.app.data.model.OrderPaymentCollectionResponse>(400, json.toResponseBody("application/json".toMediaTypeOrNull()))
        coEvery { api.collectPayment(any(), any()) } returns errorResponse
