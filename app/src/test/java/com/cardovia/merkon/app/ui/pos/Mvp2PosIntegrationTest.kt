...
class Mvp2PosIntegrationTest {
    ...
    fun testCheckoutManualOnly() = runTest {
        val reqSlot = io.mockk.slot<com.cardovia.merkon.app.data.model.ManualPosOrderRequest>()
    ...
    fun testCheckoutMixed() = runTest {
        val reqSlot = io.mockk.slot<com.cardovia.merkon.app.data.model.ManualPosOrderRequest>()
        coEvery { manualPosOrderRepository.submitOrder(capture(reqSlot)) } returns mockk(relaxed = true)
        coEvery { menuRepository.quoteItem(any(), any()) } returns com.cardovia.merkon.app.data.model.ItemQuoteResponseDto(menuItemId = 1L, name = "T", quantity = 1, baseUnitPrice = java.math.BigDecimal.TEN, baseTotal = java.math.BigDecimal.TEN, unitAdjustmentTotal = java.math.BigDecimal.ZERO, unitTotal = java.math.BigDecimal.TEN, total = java.math.BigDecimal.TEN)
        coEvery { promotionRepository.quoteCart(any()) } returns com.cardovia.merkon.app.data.model.OrderPricingPreview(java.math.BigDecimal.TEN, emptyList(), emptyList(), emptyList(), java.math.BigDecimal.TEN)

        viewModel.addToCart(com.cardovia.merkon.app.data.model.MenuItem(id = 1L, nombre = "T", categoria = "T", precio = java.math.BigDecimal.TEN))
        testDispatcher.scheduler.advanceUntilIdle()
    ...
    fun testCheckoutClearsCart() = runTest {
        ...
        coEvery { manualPosOrderRepository.submitOrder(any()) } returns com.cardovia.merkon.app.data.model.ManualPosOrderResponse(
            id = 1L,
            requestId = "req",
            result = com.cardovia.merkon.app.data.model.OrderResult.CREATED,
            orderSource = "POS",
            createdByUserId = 1L,
            fulfillmentType = com.cardovia.merkon.app.data.model.FulfillmentType.PICKUP,
        ...
        val manualCartFlow = manualCartField.get(viewModel) as kotlinx.coroutines.flow.MutableStateFlow<List<com.cardovia.merkon.app.data.model.ManualCartLine>>
        val currentCartField = PosViewModel::class.java.getDeclaredField("_currentCart")
        currentCartField.isAccessible = true
        val currentCartFlow = currentCartField.get(viewModel) as kotlinx.coroutines.flow.MutableStateFlow<List<com.cardovia.merkon.app.data.model.QuotedCartLine>>
    ...
    fun testManualMutationInvalidatesRequestId() = runTest {
        ...
        val reqSlot1 = io.mockk.slot<com.cardovia.merkon.app.data.model.ManualPosOrderRequest>()
        ...
        val reqSlot2 = io.mockk.slot<com.cardovia.merkon.app.data.model.ManualPosOrderRequest>()
