...
import com.cardovia.merkon.app.data.model.OrderPaymentCollectionResponse
import com.cardovia.merkon.app.data.model.OrderPaymentCollectionRequest
...
class OperationalOrderDtoTest {
    ...
    fun `ManualPosOrderRequest ON_DELIVERY serialization - Gson produces null paymentMethod and correct timing`() {
        val req = com.cardovia.merkon.app.data.model.ManualPosOrderRequest(
            requestId = "req-od-1",
    ...
    fun `ManualPosOrderResponse ON_DELIVERY deserialization - realistic Backend PR 41 payload`() {
        ...
        assertEquals("ANDROID_MANUAL", response.orderSource)
        assertEquals(com.cardovia.merkon.app.data.model.FulfillmentType.DELIVERY, response.fulfillmentType)
        assertNull("paymentMethod must be null", response.paymentMethod)
        assertEquals(com.cardovia.merkon.app.data.model.OrderPaymentTiming.ON_DELIVERY, response.paymentTiming)
    ...
    fun `OperationalOrderSummaryDto deserialization with paymentTiming and requiresPaymentCollection`() {
        ...
        assertEquals(com.cardovia.merkon.app.data.model.OrderPaymentTiming.ON_DELIVERY, summary.paymentTiming)
    ...
    fun `OperationalOrderDetailDto deserialization with settled ON_DELIVERY fields`() {
        ...
        assertEquals(com.cardovia.merkon.app.data.model.PaymentMethod.CASH, detail.paymentMethod)
        assertEquals(com.cardovia.merkon.app.data.model.OrderPaymentTiming.ON_DELIVERY, detail.paymentTiming)
    ...
    fun `OrderPaymentCollectionRequest serialization via Gson`() {
        val req = com.cardovia.merkon.app.data.model.OrderPaymentCollectionRequest(
            paymentMethod = com.cardovia.merkon.app.data.model.PaymentMethod.TRANSFER,
            cashDenomination = null
        )
        val json = gson.toJson(req)
        val obj = com.google.gson.JsonParser.parseString(json).asJsonObject
    ...
    fun `OrderPaymentCollectionResponse deserialization with all required fields`() {
        ...
        """
        val response = gson.fromJson(json, com.cardovia.merkon.app.data.model.OrderPaymentCollectionResponse::class.java)

        assertEquals(55L, response.orderId)
        assertEquals("READY", response.previousStatus)
        assertEquals("COMPLETED", response.currentStatus)
        assertEquals(com.cardovia.merkon.app.data.model.OrderPaymentTiming.ON_DELIVERY, response.paymentTiming)
        assertEquals(com.cardovia.merkon.app.data.model.PaymentMethod.TRANSFER, response.paymentMethod)
