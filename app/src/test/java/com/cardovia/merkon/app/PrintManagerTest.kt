...
class PrintManagerTest {
    ...
    fun `idempotency by orderId and requestId`() = runTest {
        val job = PrintJobEntity("job-1", "req-1", com.cardovia.merkon.app.data.model.PrintDocumentType.ORDER, 1L, null, PrintJobStatus.PENDING, null, 0, 0, null, null)
        coEvery { repository.enqueuePrint(com.cardovia.merkon.app.data.model.PrintDocumentType.ORDER, 1L, "req-1", null) } returns job

        manager.enqueuePrintJob(com.cardovia.merkon.app.data.model.PrintDocumentType.ORDER, 1L, "req-1")
        advanceUntilIdle()

        coVerify(exactly = 1) { repository.enqueuePrint(com.cardovia.merkon.app.data.model.PrintDocumentType.ORDER, 1L, "req-1", null) }
    }
    ...
    fun `FAILED retry becomes PRINTED and clears lastError`() = runTest {
        val job = PrintJobEntity("job-1", "req-1", com.cardovia.merkon.app.data.model.PrintDocumentType.ORDER, 1L, null, PrintJobStatus.FAILED, "error", 0, 0, null, null)
    ...
    fun `failed REPRINT keeps parent PRINTED`() = runTest {
        val job = PrintJobEntity("job-1", "req-1", com.cardovia.merkon.app.data.model.PrintDocumentType.ORDER, 1L, null, PrintJobStatus.PRINTED, null, 0, 0, 12345L, null)
    ...
    fun `reprintOrder with no local job ensures REPRINT_READY and processes REPRINT`() = runTest {
        coEvery { repository.getJobByDocument(com.cardovia.merkon.app.data.model.PrintDocumentType.ORDER, 123L) } returns null
        val newJob = PrintJobEntity("job-new", "hist-1", com.cardovia.merkon.app.data.model.PrintDocumentType.ORDER, 123L, null, PrintJobStatus.REPRINT_READY, null, 1L, 1L, null, null)
        coEvery { repository.getJobById("job-new") } returns newJob

        val requestIdSlot = slot<String>()
        coEvery { repository.ensureReprintReadyJob(com.cardovia.merkon.app.data.model.PrintDocumentType.ORDER, 123L, capture(requestIdSlot)) } returns newJob
    ...
    fun `reprintOrder with activeAttemptId != null returns AlreadyProcessing`() = runTest {
        val existingJob = PrintJobEntity("job-ex", "hist-2", com.cardovia.merkon.app.data.model.PrintDocumentType.ORDER, 888L, null, PrintJobStatus.REPRINT_READY, null, 1L, 1L, null, "att-running")
        coEvery { repository.getJobByDocument(com.cardovia.merkon.app.data.model.PrintDocumentType.ORDER, 888L) } returns existingJob
