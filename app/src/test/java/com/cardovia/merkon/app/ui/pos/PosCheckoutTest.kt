package com.cardovia.merkon.app.ui.pos
...
import com.cardovia.merkon.app.data.repository.IManualPosOrderRepository
import com.cardovia.merkon.app.data.repository.IMenuRepository
import com.cardovia.merkon.app.data.repository.IPromotionRepository
...
class PosCheckoutTest {
    ...
    fun `checkout maps BUSINESS_DAY_CLOSED to business specific message`() = runTest(testDispatcher) {
        fillCart()
        viewModel.updateFulfillmentType(com.cardovia.merkon.app.data.model.FulfillmentType.PICKUP)
        viewModel.updatePickupName("Test Name")
        viewModel.updatePaymentMethod(com.cardovia.merkon.app.data.model.PaymentMethod.CASH)

        // Mock the repository to return BusinessDayClosedException
        fakeManualRepo.shouldFailWithApiError = com.cardovia.merkon.app.data.api.BusinessDayClosedException()
        viewModel.cobrarOrden()
        testDispatcher.scheduler.advanceUntilIdle()

        val state = (viewModel.uiState.value as com.cardovia.merkon.app.ui.pos.PosUiState.Success).checkoutState
    ...
    private lateinit var printJobRepository: com.cardovia.merkon.app.data.repository.IPrintJobRepository
    private lateinit var printManager: com.cardovia.merkon.app.PrintManager
    ...
    fun `testCheckout_enqueuePrintJobFails_producesConfirmedWithPrintWarning`() = runTest {
        ...
        io.mockk.coEvery {
            printManager.enqueuePrintJob(any<com.cardovia.merkon.app.data.model.PrintDocumentType>(), any<Long>(), any<String>(), any())
    ...
    fun `testCheckout_enqueueSuspensionDelaysSuccessAndKeepsCartIntact`() = runTest {
        viewModel.updateFulfillmentType(com.cardovia.merkon.app.data.model.FulfillmentType.DELIVERY)
        viewModel.updateDeliveryAddress("Av. Falsa 123")
        viewModel.updateCashDenomination(java.math.BigDecimal("200.00"))


        val maki = com.cardovia.merkon.app.data.model.MenuItem(1L, "Maki", "Rolls", java.math.BigDecimal("100.00"), "", "")
        ...
        io.mockk.coEvery {
            ...
            com.cardovia.merkon.app.data.local.PrintJobEntity(
                "job-1", "req-1", com.cardovia.merkon.app.data.model.PrintDocumentType.ORDER, 1L, null, com.cardovia.merkon.app.data.model.PrintJobStatus.PENDING, null, 0L, 0L, null, null
            )
        }
        viewModel.cobrarOrden()
    ...
    fun `test PICKUP preserves ON_DELIVERY`() = runTest(testDispatcher) {
        viewModel.updateFulfillmentType(com.cardovia.merkon.app.data.model.FulfillmentType.DELIVERY)
        viewModel.updatePaymentTiming(com.cardovia.merkon.app.data.model.OrderPaymentTiming.ON_DELIVERY)
        viewModel.updateFulfillmentType(com.cardovia.merkon.app.data.model.FulfillmentType.PICKUP)

        testDispatcher.scheduler.advanceUntilIdle()
        val meta = viewModel.uiState.value as com.cardovia.merkon.app.ui.pos.PosUiState.Success

        assertEquals(com.cardovia.merkon.app.data.model.FulfillmentType.PICKUP, meta.fulfillmentType)
        assertEquals(com.cardovia.merkon.app.data.model.OrderPaymentTiming.ON_DELIVERY, meta.paymentTiming)
    ...
    fun `test deferred timing clears payment fields and immediate restores CASH`() = runTest(testDispatcher) {
        ...
        viewModel.updatePaymentTiming(com.cardovia.merkon.app.data.model.OrderPaymentTiming.ON_DELIVERY)
        testDispatcher.scheduler.advanceUntilIdle()
        var state = viewModel.uiState.value as com.cardovia.merkon.app.ui.pos.PosUiState.Success
        assertEquals(com.cardovia.merkon.app.data.model.OrderPaymentTiming.ON_DELIVERY, state.paymentTiming)
        assertNull(state.paymentMethod)
        assertNull(state.cashDenomination)

        viewModel.updatePaymentTiming(com.cardovia.merkon.app.data.model.OrderPaymentTiming.IMMEDIATE)
        testDispatcher.scheduler.advanceUntilIdle()
        state = viewModel.uiState.value as com.cardovia.merkon.app.ui.pos.PosUiState.Success
        assertEquals(com.cardovia.merkon.app.data.model.OrderPaymentTiming.IMMEDIATE, state.paymentTiming)
        assertEquals(com.cardovia.merkon.app.data.model.PaymentMethod.CASH, state.paymentMethod)
        assertNull(state.cashDenomination)
    }
    ...
    fun `test PICKUP ON_DELIVERY request preserves pickup fulfillment and unpaid payment fields`() = runTest(testDispatcher) {
        ...
        val request = fakeManualRepo.lastRequest
        assertNotNull(request)
        assertEquals(com.cardovia.merkon.app.data.model.FulfillmentType.PICKUP, request?.fulfillmentType)
        assertEquals(com.cardovia.merkon.app.data.model.OrderPaymentTiming.ON_DELIVERY, request?.paymentTiming)
    ...
    fun `test DELIVERY ON_DELIVERY request preserves delivery fulfillment and unpaid payment fields`() = runTest(testDispatcher) {
        ...
        val request = fakeManualRepo.lastRequest
        assertNotNull(request)
        assertEquals(com.cardovia.merkon.app.data.model.FulfillmentType.DELIVERY, request?.fulfillmentType)
        assertEquals(com.cardovia.merkon.app.data.model.OrderPaymentTiming.ON_DELIVERY, request?.paymentTiming)
        assertNull(request?.paymentMethod)
        assertNull(request?.cashDenomination)
    ...
    fun `test DELIVERY immediate CARD invalid-state prevention`() = runTest(testDispatcher) {
        ...
        viewModel.updateFulfillmentType(com.cardovia.merkon.app.data.model.FulfillmentType.PICKUP)
        viewModel.updatePaymentMethod(com.cardovia.merkon.app.data.model.PaymentMethod.CARD)
...
class FakeManualPosOrderRepository : IManualPosOrderRepository {
    override suspend fun createOpenSale(request: com.cardovia.merkon.app.data.model.OpenSaleRequest): com.cardovia.merkon.app.data.model.OpenSaleResponse { throw Exception("stub") }
