...
import com.cardovia.merkon.app.data.model.OperationalOrderSummaryDto
...
class KitchenViewModelTest {
    ...
    private lateinit var mockApi: MerkonApi
    private lateinit var mockRepository: IOrderRepository
    private lateinit var mockOperationalRepo: com.cardovia.merkon.app.data.repository.IOperationalOrderRepository
    ...
    fun `acceptOperationalOrder prepares before fetching detail for printing`() = runTest {
        ...
        val dummyDetail = mockk<com.cardovia.merkon.app.data.model.OperationalOrderDetailDto>(relaxed = true)
    ...
    private fun makeOperationalOrderSummary(
        ...
        paymentMethod: com.cardovia.merkon.app.data.model.PaymentMethod? = com.cardovia.merkon.app.data.model.PaymentMethod.CASH,
        cashDenomination: java.math.BigDecimal? = null
    ) = com.cardovia.merkon.app.data.model.OperationalOrderSummaryDto(
        id = id,
        ...
        paymentTiming = com.cardovia.merkon.app.data.model.OrderPaymentTiming.ON_DELIVERY,
        pickupName = null,
    ...
    fun `test closeCollectionConfirmation ignores if in flight`() = runTest(testDispatcher) {
        ...
        val deferred = kotlinx.coroutines.CompletableDeferred<com.cardovia.merkon.app.data.model.OrderPaymentCollectionResponse>()
        coEvery { mockOperationalRepo.collectPayment(any(), any(), any()) } coAnswers { deferred.await() }

        viewModel.fetchBackendOrders()
        advanceUntilIdle()

        viewModel.submitCollection(1L, com.cardovia.merkon.app.data.model.PaymentMethod.CARD, null)
        advanceUntilIdle()
    ...
    fun `test submitCollection ignores if already in flight`() = runTest(testDispatcher) {
        ...
        coVerify(exactly = 1) { mockOperationalRepo.collectPayment(1L, com.cardovia.merkon.app.data.model.PaymentMethod.CARD, null) }
    ...
    fun `test submitCollection sets error if order missing`() = runTest(testDispatcher) {
        viewModel.submitCollection(999L, com.cardovia.merkon.app.data.model.PaymentMethod.CARD, null)
    ...
    fun `test submitCollection validates CASH amount correctly (valid)`() = runTest(testDispatcher) {
        ...
        viewModel.submitCollection(1L, com.cardovia.merkon.app.data.model.PaymentMethod.CASH, java.math.BigDecimal("150.00"))
        advanceUntilIdle()

        coVerify { mockOperationalRepo.collectPayment(1L, com.cardovia.merkon.app.data.model.PaymentMethod.CASH, java.math.BigDecimal("150.00")) }
    }
    ...
    fun `test submitCollection validates CASH amount correctly (invalid)`() = runTest(testDispatcher) {
        ...
        viewModel.submitCollection(1L, com.cardovia.merkon.app.data.model.PaymentMethod.CASH, java.math.BigDecimal("50.00"))
        advanceUntilIdle()
    ...
    fun `test submitCollection maps ApiException on validation failure`() = runTest(testDispatcher) {
        val order = makeOperationalOrderSummary(id = 1L, status = "READY", requiresPaymentCollection = true, total = java.math.BigDecimal("100.00"))
        coEvery { mockApi.getOperationalActiveOrders() } returns retrofit2.Response.success(listOf(order))
        coEvery { mockOperationalRepo.collectPayment(any(), any(), any()) } throws com.cardovia.merkon.app.data.api.ApiException("ORDER_INVALID_TRANSITION", "Invalid transition")
        viewModel.fetchBackendOrders()
        advanceUntilIdle()
    ...
    fun `test submitCollection maps ApiException on business rule failure`() = runTest(testDispatcher) {
        val order = makeOperationalOrderSummary(id = 1L, status = "READY", requiresPaymentCollection = true, total = java.math.BigDecimal("100.00"))
        coEvery { mockApi.getOperationalActiveOrders() } returns retrofit2.Response.success(listOf(order))
        coEvery { mockOperationalRepo.collectPayment(any(), any(), any()) } throws com.cardovia.merkon.app.data.api.ApiException("ORDER_INVALID_PAYMENT_COLLECTION_REQUEST", "Datos insuficientes")
        viewModel.fetchBackendOrders()
        advanceUntilIdle()
    ...
    fun `test submitCollection mapping handles unexpected ApiException code`() = runTest(testDispatcher) {
        val order = makeOperationalOrderSummary(id = 1L, status = "READY", requiresPaymentCollection = true, total = java.math.BigDecimal("100.00"))
        coEvery { mockApi.getOperationalActiveOrders() } returns retrofit2.Response.success(listOf(order))
        coEvery { mockOperationalRepo.collectPayment(any(), any(), any()) } throws com.cardovia.merkon.app.data.api.ApiException("UNKNOWN_ERROR", "Something broke")
        viewModel.fetchBackendOrders()
        advanceUntilIdle()
    ...
    fun `test submitCollection on IOException with Success outcome clears state`() = runTest(testDispatcher) {
        ...
        coEvery { mockOperationalRepo.collectPayment(any(), any(), any()) } throws java.io.IOException("Network error")
        val detail = mockk<com.cardovia.merkon.app.data.model.OperationalOrderDetailDto>(relaxed = true) {
            coEvery { status } returns "COMPLETED"
            coEvery { requiresPaymentCollection } returns false
            coEvery { paymentMethod } returns com.cardovia.merkon.app.data.model.PaymentMethod.CARD
