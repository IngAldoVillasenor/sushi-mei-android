...
class PosViewModelTest {
    ...
    fun `OpenSale first attempt throws network error, retry SAME payload captures same requestId`() = runTest {
        val requests = mutableListOf<com.cardovia.merkon.app.data.model.OpenSaleRequest>()
    ...
    fun `OpenSale CREATED response captures currentPrintJobId`() = runTest {
        ...
        val job = com.cardovia.merkon.app.data.local.PrintJobEntity("job-123", "req-1", PrintDocumentType.ORDER, 100L, null, PrintJobStatus.PENDING, null, 1L, 1L, null, null)
    ...
    fun `submitVoidOrder on failure keeps order in list and exposes recoverable error`() = runTest {
        ...
        coEvery { operationalOrderRepository.voidOrder(77L, "Error reason") } throws
            com.cardovia.merkon.app.data.api.ApiException("ORDER_INVALID_TRANSITION", "Cannot void")
    ...
    fun `duplicate collection tap results in exactly one repository collectPayment call`() = runTest {
        ...
        val deferred = CompletableDeferred<com.cardovia.merkon.app.data.model.OrderPaymentCollectionResponse>()
