...
import com.cardovia.merkon.app.PrintManager
import com.cardovia.merkon.app.data.repository.IPrintJobRepository
...
import com.cardovia.merkon.app.data.repository.IOperationalOrderRepository
...
sealed interface CheckoutState {
    ...
    data class OpenSaleSuccess(val response: com.cardovia.merkon.app.data.model.OpenSaleResponse) : CheckoutState
    data class OpenSaleConfirmedWithPrintWarning(val response: com.cardovia.merkon.app.data.model.OpenSaleResponse, val orderId: Long, val requestId: String, val message: String) : CheckoutState
}
...
sealed interface ActiveOrderManagementState {
    ...
    data class Content(
        val orders: List<com.cardovia.merkon.app.data.model.OperationalOrderSummaryDto>,
...
sealed interface PosUiState {
    ...
    data class Success(
        ...
        val manualCart: List<com.cardovia.merkon.app.data.model.ManualCartLine> = emptyList(),
...
class PosViewModel(
    ...
    val currentPrintState: StateFlow<CurrentPrintUiState> = _currentPrintJobId.flatMapLatest { jobId ->
        if (jobId == null) {
            ...
            combine(
                ...
                val internalCopyAttempts = attempts.filter { it.type == com.cardovia.merkon.app.data.model.PrintAttemptType.INTERNAL_COPY }
                ...
                when (latestInternalCopy.status) {
                    com.cardovia.merkon.app.data.model.PrintAttemptStatus.PRINTING -> CurrentPrintUiState.InternalCopyPrinting(job.id, job.documentId)
                    com.cardovia.merkon.app.data.model.PrintAttemptStatus.FAILED, com.cardovia.merkon.app.data.model.PrintAttemptStatus.INTERRUPTED -> CurrentPrintUiState.InternalCopyFailed(job.id, job.documentId, latestInternalCopy.error ?: "Error desconocido")
                    com.cardovia.merkon.app.data.model.PrintAttemptStatus.SUCCEEDED -> CurrentPrintUiState.InternalCopyPrinted
                }
            }
        }
    ...
    private val _manualCart = MutableStateFlow<List<com.cardovia.merkon.app.data.model.ManualCartLine>>(emptyList())
    ...
    val uiState: StateFlow<PosUiState> = combine(
        combine(
            ...
            CatalogState(
                allProducts = args[0] as List<com.cardovia.merkon.app.data.model.MenuItem>,
                selectedCategory = args[1] as String?,
                cart = args[2] as List<com.cardovia.merkon.app.data.model.ConfiguredProduct>,
                activePromotions = args[3] as List<com.cardovia.merkon.app.data.model.Promotion>,
                promotionLoadError = args[4] as String?,
                manualCart = args[5] as List<com.cardovia.merkon.app.data.model.ManualCartLine>
            )
        },
    ...
    private data class CatalogState(
        ...
        val manualCart: List<com.cardovia.merkon.app.data.model.ManualCartLine>,
    ...
    fun cobrarOrden() {
        ...
        val requestSnapshot = ManualPosOrderRequest(
            ...
            manualLines = manualItems.map {
                com.cardovia.merkon.app.data.model.ManualPricedLineRequest(
                    lineKey = it.lineKey,
        ...
        viewModelScope.launch {
            try {
                ...
                if (response.result == OrderResult.CREATED || response.result == OrderResult.ALREADY_CREATED) {
                    try {
                        val job = printManager.enqueuePrintJob(com.cardovia.merkon.app.data.model.PrintDocumentType.ORDER, response.id, response.requestId)
    ...
    fun retryPrintRegistration(orderId: Long, requestId: String, response: ManualPosOrderResponse) {
        viewModelScope.launch {
            try {
                val job = printManager.enqueuePrintJob(com.cardovia.merkon.app.data.model.PrintDocumentType.ORDER, orderId, requestId)
                _currentPrintJobId.value = job.id
                _checkoutState.value = CheckoutState.Success(response)
            } catch (e: Exception) {
                _checkoutState.value = CheckoutState.ConfirmedWithPrintWarning(
    ...
    fun retryOpenSalePrintRegistration(orderId: Long, requestId: String, response: com.cardovia.merkon.app.data.model.OpenSaleResponse) {
        viewModelScope.launch {
            try {
                val job = printManager.enqueuePrintJob(com.cardovia.merkon.app.data.model.PrintDocumentType.ORDER, orderId, requestId)
                _currentPrintJobId.value = job.id
    ...
    fun addManualLine(description: String, unitAmount: java.math.BigDecimal) {
        val newLine = com.cardovia.merkon.app.data.model.ManualCartLine(
    ...
    fun submitOpenSale(
        ...
        val finalDenom = if (paymentMethod == com.cardovia.merkon.app.data.model.PaymentMethod.CASH) cashDenomination else null
        ...
        viewModelScope.launch {
            try {
                val request = com.cardovia.merkon.app.data.model.OpenSaleRequest(
                    requestId = requestId,
                    description = description,
                    amount = amount,
                ...
                if (response.result == "CREATED" || response.result == "ALREADY_CREATED") {
                    ...
                    try {
                        val job = printManager.enqueuePrintJob(
                            com.cardovia.merkon.app.data.model.PrintDocumentType.ORDER,
                            response.id,
                            request.requestId
                        )
                        _currentPrintJobId.value = job.id
    ...
    private suspend fun refreshAfterVoidFailure(sessionAtStart: Long, fallbackOrders: List<com.cardovia.merkon.app.data.model.OperationalOrderSummaryDto>, errorMessage: String) {
        val repo = operationalOrderRepository ?: return
        try {
            val raw = repo.getOperationalActiveOrders()
            val filtered = raw.filter { order ->
        ...
        viewModelScope.launch {
            try {
                ...
                _activeOrderManagementState.value = current.copy(
                    ...
                    collectionError = com.cardovia.merkon.app.ui.shared.CollectionOrchestrator.mapCollectionApiError(e)
                )
            } catch (e: java.io.IOException) {
                if (currentSession != activeOrderSessionGeneration) return@launch
                try {
