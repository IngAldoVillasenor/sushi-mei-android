...
import com.cardovia.merkon.app.data.model.OrderRecord
...
class PrintService(private val context: Context) {
    ...
    private fun imprimirOrdenLocal(out: OutputStream, order: com.cardovia.merkon.app.data.model.Order) {
        val ESC: Byte = 0x1B
    ...
    fun printClosingTicket(day: com.cardovia.merkon.app.data.model.BusinessDayResponse): Boolean {
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
            if (androidx.core.app.ActivityCompat.checkSelfPermission(context, android.Manifest.permission.BLUETOOTH_CONNECT) != android.content.pm.PackageManager.PERMISSION_GRANTED) {
                return false
            }
    ...
    private fun printConfigurationTree(
        ...
        val groupedChildren = mutableListOf<Pair<PresentationConfigKey, MutableList<com.cardovia.merkon.app.data.model.OrderConfigurationSnapshotDto>>>()
    ...
    fun formatOperationalTicket(order: com.cardovia.merkon.app.data.model.OperationalOrderDetailDto, isReprint: Boolean = false, isInternalCopy: Boolean = false): ByteArray {
        ...
        if (order.fulfillmentType == com.cardovia.merkon.app.data.model.FulfillmentType.DELIVERY) {
        ...
        } else if (order.fulfillmentType == com.cardovia.merkon.app.data.model.FulfillmentType.PICKUP) {
            out.write("Tipo: MOSTRADOR\n".toByteArray())
        ...
        if (order.paymentTiming == com.cardovia.merkon.app.data.model.OrderPaymentTiming.ON_DELIVERY && order.requiresPaymentCollection && order.paymentMethod == null) {
            // Pending pay-on-delivery
            out.write("Pago: Contra entrega\n".toByteArray())
        } else if (order.paymentMethod != null) {
            out.write("Pago: ${order.paymentMethod.name}\n".toByteArray())
            if (order.paymentTiming == com.cardovia.merkon.app.data.model.OrderPaymentTiming.ON_DELIVERY && !order.requiresPaymentCollection) {
        ...
        if (order.fulfillmentType == com.cardovia.merkon.app.data.model.FulfillmentType.DELIVERY && order.paymentMethod == com.cardovia.merkon.app.data.model.PaymentMethod.CASH && order.cashDenomination != null) {
