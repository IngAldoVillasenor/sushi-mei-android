...
class PrintManager(
    ...
    suspend fun enqueuePrintJob(
        documentType: com.cardovia.merkon.app.data.model.PrintDocumentType,
        documentId: Long,
        requestId: String,
        ...
        val attemptType = when (job.status) {
            com.cardovia.merkon.app.data.model.PrintJobStatus.PRINTED -> PrintAttemptType.REPRINT
            com.cardovia.merkon.app.data.model.PrintJobStatus.FAILED,
            com.cardovia.merkon.app.data.model.PrintJobStatus.INTERRUPTED -> PrintAttemptType.RETRY
    ...
    suspend fun reprintOrder(orderId: Long): ReprintStartResult {
        return try {
            val existingJob = printJobRepository.getJobByDocument(com.cardovia.merkon.app.data.model.PrintDocumentType.ORDER, orderId)
            if (existingJob != null) {
                if (existingJob.activeAttemptId != null) {
                    return ReprintStartResult.AlreadyProcessing(existingJob.id)
                }
                when (existingJob.status) {
                    com.cardovia.merkon.app.data.model.PrintJobStatus.PRINTED,
                    com.cardovia.merkon.app.data.model.PrintJobStatus.REPRINT_READY -> {
                        coroutineScope.launch { processPrintJob(existingJob.id, PrintAttemptType.REPRINT) }
                        ReprintStartResult.Started(existingJob.id)
                    }
                    com.cardovia.merkon.app.data.model.PrintJobStatus.PENDING,
                    com.cardovia.merkon.app.data.model.PrintJobStatus.PRINTING -> {
                        ReprintStartResult.AlreadyProcessing(existingJob.id)
                    }
                    com.cardovia.merkon.app.data.model.PrintJobStatus.FAILED,
                    com.cardovia.merkon.app.data.model.PrintJobStatus.INTERRUPTED -> {
    ...
    private suspend fun processPrintJob(jobId: String, attemptType: PrintAttemptType) {
        try {
            ...
            try {
                ...
                val printed = if (job.documentType == com.cardovia.merkon.app.data.model.PrintDocumentType.ORDER) {
                    ...
                    val day = com.cardovia.merkon.app.data.api.NetworkModule.configuredGson.fromJson(job.snapshotPayload, com.cardovia.merkon.app.data.model.BusinessDayResponse::class.java)
