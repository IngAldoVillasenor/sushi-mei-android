...
class PrintService(private val context: Context) {
    ...
    fun printOperationalTicket(order: OperationalOrderDetailDto, isReprint: Boolean = false): Boolean {
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
            if (androidx.core.app.ActivityCompat.checkSelfPermission(context, android.Manifest.permission.BLUETOOTH_CONNECT) != android.content.pm.PackageManager.PERMISSION_GRANTED) {
                return false
            }
        ...
        return try {
            ...
            val formattedText = formatOperationalTicket(order, isReprint)
    ...
    fun formatOperationalTicket(order: com.restaurant.sushimei.frontend.data.model.OperationalOrderDetailDto, isReprint: Boolean = false): ByteArray {
